...
void Block::Rotate_CW(){
  ...
  for(int i = 0; i < 4; i++)
    {
      if(!_pBoard->isAvailable((Coordinate){
                             penetrateWall(_refCoord.xCoord + _blockCoords[newRState][i].xCoord, _pBoard->getWidth()),
  ...
  for(int i = 0; i < 4; i++)
    {
      newCoord = (Coordinate){penetrateWall(_refCoord.xCoord + _blockCoords[newRState][i].xCoord, _pBoard->getWidth()),
                  _refCoord.yCoord - _blockCoords[newRState][i].yCoord};
  ...
  for(list<Cell*>::iterator it = _cells.begin(); it != _cells.end(); it++)
    {
      ...
      if(!_pBoard->isAvailable((Coordinate){penetrateWall(coord.xCoord + 1, _pBoard->getWidth()), coord.yCoord}))
	{
	  return;
	}
    }
  ...
  for(list<Cell*>::iterator it = _cells.begin(); it != _cells.end(); it++)
    {
      // Compute the coordinate of the dropped cell
      oldCoord = (*it)->getCoord();
      newCoords.push_back((Coordinate){penetrateWall(oldCoord.xCoord + 1, _pBoard->getWidth()), oldCoord.yCoord});
...
int Block::penetrateWall(int x, int boardWidth)
{
  int result = x % boardWidth;
  return (result >= 0 ? result : (result + boardWidth)); 
}
